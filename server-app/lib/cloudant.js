...
function getCount() {
   return new Promise((resolve, reject) => {
        db.get(db_name, (err, data) => {
            if (err) {
               reject(err);
            } else {
		resolve({ data: JSON.stringify(data), statusCode: 200});
	    }
	});
   });
...
function deleteAll( rev) {
    return new Promise((resolve, reject) => {
        db.get( (err, document) => {
            if (err) {
                resolve(err.statusCode);
            } else {
                db.destroy( document._rev, (err) => {
                    if (err) {
                        reject(err);
                    } else {
                        resolve(200);
 ...
 */
function create(name, eid, workloc, locality, contactnum, information) {
    return new Promise((resolve, reject) => {
        let itemId = uuidv4();
 ...
 * when the call to the DB completes
 */
function update(id, workloc, contactnum, locality, information) {
   let whenUpdated = Date.now();
    return new Promise((resolve, reject) => {
        db.get(id, (err, document) => {
            if (err) {
		     ...
		     id: id,
                    ...
                    whenUpdated : whenUpdated
                                // Specifiying the _rev turns this into an update
                }
                if (information) {item["information"] = information} else {item["information"] = document.information};
                if (workloc) {item["workloc"] = workloc} else {item["workloc"] = document.workloc};
                if (locality) {item["locality"] = locality} else {item["locality"] = document.locality};
                if (contactnum) {item["contactnum"] = contact} else {item["contactnum"] = document.contactnum};
