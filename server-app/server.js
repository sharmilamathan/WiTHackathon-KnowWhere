...
function post_process_assistant(result) {
  ...
  if (result.intents.length > 0 ) {
    result.entities.forEach(item => {
      if ((item.entity == "Employee") &&  (item.confidence > 0.3)) {
        resource = item.value
      }
    })
  }
...
app.get('/api/employees', (req, res) => {
  ...
  cloudant
	...
	processed_result = data.data;
        if (data.data != "[]") {
          processed_result = JSON.parse(data.data);
	  processed_result.forEach(item => { 
	       delete item._id;
               //delete item.id;
                delete item._rev;
                delete item.whenCreated; 
		delete item.whenUpdated;
          	cloudant.getCount();
          	console.log(item);
	})
...
app.post('/api/employee', (req, res) => {
 ...
 //console.log(reqarray.length);
  ...
  if (!req.body.name) {
    return res.status(422).json({ errors: "Name of Employee must be provided"});
  }
  if (!req.body.contactnum) {
    return res.status(422).json({ errors: "A Mobile number must be provided"});
  }
  if (!req.body.eid) {
    return res.status(422).json({ errors: "Employee Id must be provided"});
  }
  if (!req.body.workloc) {
    return res.status(422).json({ errors: "Working Location must be provided"});
  }
  if (!req.body.locality) {
    return res.status(422).json({ errors: "Working must be provided"});
  }
  const name = req.body.name;
  const eid = req.body.eid;
  const workloc = req.body.workloc;
  ...
  cloudant
    .create( name, eid, workloc, locality, contactnum, information)
    .then(data => {
...
app.patch('/api/employee/:id', (req, res) => {
  ...
  cloudant
    .update(req.params.id, workloc, contactnum, locality, information)
    .then(data => {
      if (data.statusCode != 200) {
        res.sendStatus(data.statusCode)
      } else {
